...
protected:
   ...
   int getHeightHelper(std::shared_ptr<BinaryNode<ItemType>> subTreePtr) 
const;
   int getNumberOfNodesHelper(std::shared_ptr<BinaryNode<ItemType>> 
subTreePtr) const;
...
rightTreePtr);
   ...
   void display(const ItemType& root, int blanks);
   ...
   //------------------------------------------------------------
   bool replace(const ItemType item, const ItemType itemReplace);
}; // end BinaryNodeTree
